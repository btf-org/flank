...
#error "Unsupported platform"
        ...
        if(event_fd == server_fd){
            ...
            if (strcmp(path, "/iflank") == 0
                ...
                if (epoll_ctl(ep, EPOLL_CTL_ADD, from_iflank_pipe_rw[0], &iflank_ev) == -1) {
                    perror("epoll_ctl");
                    exit(1);
                }
#elif defined(__APPLE__) || defined(__FreeBSD__)
                struct kevent iflank_ev;
                EV_SET(&iflank_ev, from_iflank_pipe_rw[0], EVFILT_READ, EV_ADD, 0, 0, NULL);
...
#elif defined(__APPLE__) || defined(__FreeBSD__)
            struct kevent ev;
            EV_SET(&ev, from_iflank_pipe_rw[0], EVFILT_READ, EV_DELETE, 0, 0, NULL);
